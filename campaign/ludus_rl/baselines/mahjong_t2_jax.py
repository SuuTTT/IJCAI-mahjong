"""T2-JAX -- fully on-GPU, fused KL-leashed PPO fine-tune of the MCR Mahjong SL policy.
...
DESIGN NOTES / deliberate choices (documented, honest):
  ...
  3. CRITIC.  We use a FRESH lightweight flax conv critic (predicts the return =
     score0/8 directly), trained from scratch by GAE -- the task permits this over
     porting the torch BN critic; it is far less code and the KL trust-region
     rollback protects stability regardless of early critic noise.
...
Usage:
  python -m baselines.mahjong_t2_jax --selftest        # validate the Hu checker
  python -m baselines.mahjong_t2_jax --parity 300      # flax policy vs numpy champion
  python -m baselines.mahjong_t2_jax --seed 1 --updates 100000 --resume \
...
# ==================================================================== rollout
def _act_batch(pparams, states, keyL, keyO, greedy):
    ...
    # opponents (seats 1-3) with frozen SL
    obsO = obs[:, 1:].reshape(B * 3, 38, 4, 9)
    maskO = mask[:, 1:].reshape(B * 3, N_ACT)
    logitsO = pforward(FROZEN, obsO, maskO)
    if greedy:
...
def make_train_rollout(T):
    def step_fn(carry, _):
        ...
        actions, obs0, mask0, act0, logp0, rec = _act_batch(
            carry_p[0], states, kL, kO, greedy=False)
        nstates = jax.vmap(J.step)(states, actions)
        return (nstates, key), (obs0, mask0, act0, logp0, rec)

    # carry_p is a one-element list so step_fn can see the (arg) params
    carry_p = [None]

    @jax.jit
    def rollout(pparams, walls, quans, key):
        carry_p[0] = pparams
...
def make_eval_rollout(T):
    ...
    def step_fn(carry, _):
        ...
        actions, obs0, mask0, act0, logp0, rec = _act_batch(
            carry_p[0], states, key, key, greedy=True)
...
def ppo_update(pparams, vparams, popt, vopt, obs, mask, act, oldlp, adv, ret,
               valid, beta, key, epochs, nmb, mb, clip, vcoef, entcoef, gate, maxgn):
    ...
    return pparams, vparams, popt, vopt, dict(
        kl=post_kl, pg=st[0], vloss=st[1], ent=st[2])
...
class Trainer:
    ...
    def collect_and_update(self):
        ...
        W, Q = self._walls(a.B)
        final, (obs0, mask0, act0, logp0, rec) = self.train_roll(self.pparams, W, Q, kr)
        R, _, _, _ = terminal_stats(final)
        Rj = jnp.asarray(R)
        val0 = jax.vmap(lambda o: v_forward(self.vparams, o))(obs0)     # (T,B)
        adv, ret = compute_gae(val0, rec, Rj, a.lam)
        ...
        np_, nv_, po_, vo_, stats = ppo_update(
            ...
            obsB, maskB, actB, logpB, advB, retB, valid, jnp.float32(self.beta), ku,
            a.epochs, self.nmb, a.minibatch, a.clip, a.vcoef, a.entcoef,
            a.kl_gate_frac * a.kl_target, a.max_grad_norm)
        post_kl = float(jax.device_get(stats["kl"]))
        rejected = post_kl > a.kl_target
        ...
        return dict(kl=post_kl, rejected=rejected, nvalid=nvalid,
                    pg=float(stats["pg"]), vloss=float(stats["vloss"]),
    ...
    def bench_rollout(self, reps=5):
        ...
        final, _ = self.train_roll(self.pparams, W, Q, k)   # compile + warm
        final.done.block_until_ready()
        t = time.time()
        for _ in range(reps):
            self.key, k = jax.random.split(self.key)
            final, _ = self.train_roll(self.pparams, W, Q, k)
            final.done.block_until_ready()
        dt = (time.time() - t) / reps
        sps = (a.B * a.T) / dt
    ...
    def evaluate(self):
        ...
        while len(scores) < n:
            ...
            W, Q = self._walls(a.B)
            final, kss, kcc = self.eval_roll(self.pparams, W, Q, ke)
        ...
        rec = {"step": self.update, "step_kind": "ppo_updates", "env_steps": self.env_steps,
               ...
               "beta": round(self.beta, 5), "steps_s": round(self.steps_s, 1),
        ...
        while self.update < a.updates:
            ...
            if self.update % a.log_every == 0:
                print(f"[u{self.update}] kl={st['kl']:.4f}{'(REJ)' if st['rejected'] else ''} "
                      f"beta={self.beta:.4f} pg={st['pg']:.4f} vloss={st['vloss']:.3f} "
                      f"ent={st['ent']:.3f} R={st['R_mean']:.3f} nrec={st['nvalid']} "
...
def main():
    ...
    ap.add_argument("--out", type=str, default="/root/ludus_train/mahjong_t2_jax/")
