...
## Warpath v2 status vs the full-game milestone ladder (owner tutorial)

M1 map viewer ✅ (iso three.js) · M2 movement ✅ (straight-line; A* later) ·
M3 selection/commands ✅ v2 (drag-box, per-unit orders, cmd queue) ·
M4 combat ✅ (class multipliers) · M5 buildings ✅ (adjacency; footprints later)
· M6 economy ✅ (ore/harvest/credits) · M7 production+sidebar ✅ v2
(prereq graph per RA2 Prerequisite semantics, dynamic re-lock on loss, power
model, RA2-style sidebar with locked-button reasons + progress) · M8
fog/minimap ◐ (radar-gated minimap live; fog later) · M9 skirmish AI ✅ v1
(scripted commander) · M10 replays ◻ next · M11 lockstep MP ◻ (engine already
deterministic lockstep-ready) · M12 mod support ◐ (tables are data; yaml
externalization later).
