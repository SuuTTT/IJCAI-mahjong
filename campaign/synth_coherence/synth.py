...
# ---- global config (recorded in every JSON) ----
D = 32          # input dim
K = 10          # classes
H_SUB = int(os.environ.get("H_SUB", "16"))  # hidden width of each ground-truth sub-policy
N_TRAIN = 200_000
N_TEST = 20_000
...
def make_subpolicy(rng, d, k, h, device):
    ...
    W1 = torch.tensor(rng.normal(0, 1.0 / math.sqrt(d), size=(d, h)), dtype=torch.float32, device=device)
...
def run_cell(C, eps, seeds, gpu, outdir):
    ...
    out = dict(
        ...
        n_train=N_TRAIN, n_test=N_TEST, d=D, K=K, H_sub=H_SUB,
...
def calibrate(gpu):
    ...
    for C in [1, 2, 4, 8]:
        ...
        print(f"C={C}: clean 1-teacher test acc={a:.4f}  gt_class_balance={counts.tolist()}", flush=True)
