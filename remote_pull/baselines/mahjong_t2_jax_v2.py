"""T2-JAX v2 -- fully on-GPU, fused KL-leashed PPO fine-tune of the MCR Mahjong SL policy.
...
class Trainer:
    ...
    def try_resume(self):
        ...
        print(f"[resume] update={self.update} env_steps={self.env_steps} beta={self.beta:.4f} "
              f"ret_mean={self.ret_mean:.3f} ret_std={self.ret_std:.3f}", flush=True)
        return True
        ...
        while self.update < a.updates:
            ...
            if self.update % a.log_every == 0:
                print(f"[u{self.update}] kl={st['kl']:.4f}{'(REJ)' if st['rejected'] else ''} "
                      ...
                      f"ret_std={st['ret_std']:.3f} ent={st['ent']:.3f} R={st['R_mean']:.3f} "
                      f"nrec={st['nvalid']} steps/s={self.steps_s:,.0f} "
                      f"env_steps={self.env_steps}", flush=True)
