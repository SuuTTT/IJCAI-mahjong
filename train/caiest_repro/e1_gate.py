...
class PlacementSim(Sim):
    def __init__(self, *a, cand_seat=0, claim_tau=0.0, **k):
        super().__init__(*a, **k)
    ...
    def _ask(self, seat):
        ...
        if seat == self.cand_seat and self.claim_tau > 0:
            act = apply_claim_tau(lg, mask, self.claim_tau)
        else:
            act = int(lg.argmax())
        if not mask[act]:
...
def _work(arg):
    seed, cand, ck, ccfg, ref, rk, rcfg, claim_tau = arg
    ...
    for cs in range(4):
        ...
        sim = PlacementSim(pols, seed=seed, quan=0, learner_seats=[], cnn=True,
                           cand_seat=cs, claim_tau=claim_tau)
        sim.play()
        sc = sim.scores
        cand_score = sc[cs]
    ...
    return placement_sum, pts, micro_cand
...
def main():
    ...
    args = [(a.seed0 + i, a.cand, a.cand_kind, ccfg, a.ref, a.ref_kind, rcfg, a.claim_tau)
    ...
    out = dict(cand=os.path.basename(a.cand), ref=os.path.basename(a.ref),
               claim_tau=a.claim_tau, games=ngames, seeds=len(res),
