"""e11_cond_kd_train.py — SOURCE-CONDITIONED + KD-ENSEMBLE trainer (2027 candidate).

Combines two proven levers into one student:
  (1) Source-conditioning via a 39th input plane (from e11_cond_train.py):
      base corpus (cooked_single.npz, plane=0) + Final2 finalist corpus
      (final2_cai_corpus.npz, plane=1), two-source 50/50 batches, suit/reflect/dragon
      aug, label smoothing, EMA, warmup+cosine, model selection on FINAL2 val acc,
      fused deploy pkl + traininfo.json.
  (2) Knowledge distillation from the 6 aug teachers (from e13_kd_corpus.py):
      loss = alpha*KD(student, mean_teacher_softmax_over_legal)
             + (1-alpha)*label_smoothed_CE(student, true_action).

CRITICAL: the 6 teachers are 38-PLANE models. The STUDENT is a 39-PLANE model. So each
step we build the 38-plane augmented obs batch, feed it to the teachers (38 planes) to get
the mean soft target, then append the source plane (0=base / 1=final2) to make the 39-plane
obs for the student forward. Aug never permutes the batch dim, so per-row source values stay
aligned; the source plane is spatially constant so aug does not affect it.
"""
import os, sys, argparse, time, json
sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
import numpy as np, torch, torch.nn.functional as F
import models_explore                                  # keep IN_PLANES=38 for teacher build
...
def main():
    ...
    ap.add_argument("--plane", choices=["src", "zero"], default="src")
    ap.add_argument("--alpha", type=float, default=0.7)     # KD weight
    ...
    print(f"base N={N:,} f2 N={FN:,} plane={a.plane} f2_frac={a.f2_frac} alpha={a.alpha} "
          f"seed={a.seed} steps={a.steps} ch={a.channels} blk={a.blocks}", flush=True)
    ...
    # ---- teachers: 38-PLANE models, built while models_explore.IN_PLANES == 38 ----
    assert models_explore.IN_PLANES == 38, "teachers must be built with IN_PLANES=38"
    teachers = []
    for tp in a.teachers.split(","):
        ...
        assert t.stem.weight.shape[1] == 38, "teacher stem must accept 38 planes"
        for p_ in t.parameters():
            p_.requires_grad_(False)
        teachers.append(t)
    print(f"KD teachers: {len(teachers)} alpha={a.alpha}", flush=True)

    # ---- student: 39-PLANE model, IN_PLANES flipped to 39 BEFORE instantiation ----
    models_explore.IN_PLANES = 39
    ...
    for s in range(a.steps):
        ...
        # ob/mk/y now hold the AUGMENTED 38-plane batch (teacher input); pl tracks source.
        ob38 = ob.float()                                  # 38-plane -> teachers
        ob39 = torch.cat([ob38, pl.float()], 1)            # 39-plane -> student
        if s == 0:
            assert ob38.shape[1] == 38, "teacher input must be 38 planes"
            assert ob39.shape[1] == 39, "student input must be 39 planes"
        with torch.cuda.amp.autocast():
            with torch.no_grad():
                tp_acc = None
                for t in teachers:
                    tl = t({"is_training": False,
                            "obs": {"observation": ob38, "action_mask": mk.float()}})
            ...
            logits = net({"is_training": True,
                          "obs": {"observation": ob39, "action_mask": mk.float()}})
        ...
        if s % a.valevery == 0 and s > 0:
            vf2, vb = eval_ema()
            hist.append(dict(step=s, loss=round(loss.item(), 4), kd=round(kd.item(), 4),
                             val_f2=round(vf2, 4), val_base=round(vb, 4)))
            ...
            print(f"  step {s}/{a.steps} loss {loss.item():.4f} kd {kd.item():.4f} "
                  f"valF2 {vf2:.4f} valBase {vb:.4f} best {best:.4f} ({time.time()-t0:.0f}s)", flush=True)
    vf2, vb = eval_ema()
    ...
    json.dump(dict(plane=a.plane, seed=a.seed, alpha=a.alpha, f2_frac=a.f2_frac, steps=a.steps,
                   best_val_f2=round(best, 4), hist=hist),
              open(a.out + ".traininfo.json", "w"), indent=1)
    print(f"DONE plane={a.plane} seed={a.seed} alpha={a.alpha} best_val_f2={best:.4f} -> {a.out}", flush=True)
