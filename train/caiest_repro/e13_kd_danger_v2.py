"""
e13_kd_danger_v2.py — JD-v2: danger-penalized KD with a STATE-CENTERED penalty.

v1 (e13_kd_danger.py) used pen = E[p_student(a)*danger(s,a)] over the 34 discard slots.
If the danger head's per-discard sigmoids are near-uniform within a state, that penalty is
~constant in the policy (zero useful gradient) — v1 produced identical policies at all
lambda. v2 removes the constant component:

    pen_v2 = E[ p_student(a) * (danger(s,a) - mean_legal_a danger(s,a)) ]

where the mean is over the state's LEGAL discards, so the gradient now differentiates
safe vs unsafe discards and cannot be satisfied by shrinking total discard probability.
Everything else (data, split, augs, teachers, smoothing, EMA, schedule, fused save) is
identical to e13_kd_danger.py, so lam is the only manipulated variable and the existing
jd_lam0_* students remain a valid control.
Also logs the mean per-state std of the danger sigmoids over legal discards (diagnostic).
"""
...
def main():
    ...
    print(f"N={N:,} train={len(tidx):,} val={len(vidx):,} lam_v2={a.lam_danger} seed={a.seed} "
    ...
    pen_run = None; penraw_run = None; dstd_run = None
    ...
    for s in range(a.steps):
        ...
        p_play = p_student[:, PLAY0:PLAY0 + NPLAY]
        legal_play = mk[:, PLAY0:PLAY0 + NPLAY].float()
        nleg = legal_play.sum(1).clamp(min=1.0)
        dmean = (dg34 * legal_play).sum(1) / nleg                                 # (B,)
        # per-state std of danger over legal discards (diagnostic)
        dvar = ((dg34 - dmean[:, None]) ** 2 * legal_play).sum(1) / nleg
        dstd = dvar.clamp(min=0).sqrt().mean()
        pen_raw = (p_play * dg34).sum(1).mean()                                   # v1 penalty (log only)
        pen = (p_play * (dg34 - dmean[:, None])).sum(1).mean()                    # v2: state-centered
        pen_run = pen.item() if pen_run is None else 0.99 * pen_run + 0.01 * pen.item()
        penraw_run = pen_raw.item() if penraw_run is None else 0.99 * penraw_run + 0.01 * pen_raw.item()
        dstd_run = dstd.item() if dstd_run is None else 0.99 * dstd_run + 0.01 * dstd.item()
        ...
        if s % a.valevery == 0 and s > 0:
            ...
            print(f"  step {s}/{a.steps} loss {loss.item():.4f} penV2(ema) {pen_run:.4f} "
                  f"penV1(ema) {penraw_run:.4f} dstd(ema) {dstd_run:.4f} "
    ...
    with open(a.out[:-4] + ".json", "w") as f:
        json.dump(dict(variant="v2_state_centered", lam_danger=a.lam_danger, seed=a.seed,
                       steps=a.steps, best_ema_val=round(best, 4),
                       val_chosen_danger=round(float(cd), 4),
                       pen_v2_train_ema=round(float(pen_run), 4),
                       pen_v1_train_ema=round(float(penraw_run), 4),
                       danger_std_legal_ema=round(float(dstd_run), 4),
                       seconds=round(time.time() - t0, 1)), f)
    print(f"DONE lamV2={a.lam_danger} s{a.seed} best_ema_val={best:.4f} chosen_danger={cd:.4f} "
          f"-> {a.out}", flush=True)
