...
CUR_MAIN = '/tmp/rlpair_main.pkl'      # current learner policy weights (ResBNCNN state_dict)
POOLDIR = '/tmp/rlpair_pool'           # opponent pool: 00_anchor.pkl + m_XXXX.pkl snapshots
STOP = '/root/STOP_RL'
...
def actor_play(arg):
    ...
    for g in range(n_games):
        ...
        _, scores = sim.play()
        r_raw = placement(scores, ls)                  # gate-matched terminal reward
        simb = Sim([greedy(opp)] * 4, seed=seed * 1000 + g, quan=0, learner_seats=[], cnn=True)
        _, scores_b = simb.play()
        r = r_raw - placement(scores_b, ls)            # PAIRED-WALL antithetic baseline (wall+seat luck cancels)
...
def main():
    ...
    ap.add_argument('--snapdir', default='ckpt/rl_paired')
    ap.add_argument('--tag', default='rlpair')
    ...
    print(f"[rl_paired] tag={a.tag} dev={dev} actors={a.actors} games/iter={a.actors*a.games_per_actor} "
          f"lr={a.lr} beta_kl={a.beta_kl} ent={a.ent} p_anchor={a.p_anchor}", flush=True)
    rew_hist = []
    for it in range(a.iters):
        if os.path.exists(STOP):
            print("[rl_paired] STOP_RL present -> stopping", flush=True); break
        if time.time() > deadline:
            print("[rl_paired] time cap reached -> stopping", flush=True); break
        t0 = time.time()
