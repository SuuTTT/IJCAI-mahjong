...
def main():
    ...
    ap.add_argument("--aug",type=float,default=0.8); ap.add_argument("--seed",type=int,default=0); ap.add_argument("--out",required=True)
    a=ap.parse_args()
    dev="cuda"
    ...
    N=len(ac); rng=np.random.RandomState(a.seed); perm=rng.permutation(N)
    ...
    torch.manual_seed(a.seed); net=ResBNCNN(channels=a.channels,blocks=a.blocks).to(dev)
