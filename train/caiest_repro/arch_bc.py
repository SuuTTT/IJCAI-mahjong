"""
arch_bc.py — generic BC trainer for the EXPLORATION architectures (attn/cnn_attn/gnn/resbn),
using the EXACT e11 enhanced-aug recipe (suit-perm x rank-reflect x dragon-perm), label
smoothing over legal actions, EMA weights, warmup+cosine. Same data + SAME val split (rng 12345)
as e11_train.py so val_acc is directly comparable to aug_s0 (0.887). Saves the RAW best-EMA
state_dict (attn/gnn are not BN-fuseable; deploy = research-only, load via build(kind)).

  CUDA_VISIBLE_DEVICES=0 python3 arch_bc.py --kind attn \
     --cfg d_model=192,layers=6,heads=8 --steps 90000 --out ckpt/archx/attn_s0.pkl --seed 0
"""
...
def main():
    ...
    print(f"N={N:,} train={len(tidx):,} val={len(vidx):,} kind={a.kind} cfg={cfg} "
          f"seed={a.seed} steps={a.steps} lsm={a.lsm} ema={a.ema}", flush=True)
    ...
    ema_net = build(a.kind, **cfg).to(dev)
