"""
cook_value_postact.py — POST-DISCARD value states; derived from preprocess_value.py: same per-decision extraction & cleaning as preprocess_single.py, but
...
OUT = '/root/IJCAI-mahjong/train/caiest_repro/data/cooked_value_post.npz'
KEEP = 0.5  # subsample post states (disk)
DATA = '/root/IJCAI-mahjong/train/caiest_repro/data/data.txt'

all_obs, all_mask, all_act, all_score, all_place = [], [], [], [], []
post_obs, post_score, post_place = [], [], []
import numpy as _np
_prng = _np.random.RandomState(777)
...
def flush(obs, actions, scores, pobs=None):
    place = placement_from_scores(scores)
    if pobs is not None:
        for j in range(4):
            for o in pobs[j]:
                if _prng.random() < KEEP:
                    post_obs.append(o.astype(np.int8))
                    post_score.append(np.float32(scores[j]))
                    post_place.append(np.int8(place[j]))


def main():
    obs = [[] for _ in range(4)]
    actions = [[] for _ in range(4)]
    pobs = [[] for _ in range(4)]
    agents = None
    curTile = None
    matchid = -1
        ...
        for line in f:
            ...
            if t[0] == 'Match':
                ...
                if t[2] == 'Deal':
                    ...
                    for i in range(4):
                        if i == p:
                            agents[p].request2obs(line.strip())   # updates state; returns None for self
                            pobs[p].append(agents[p]._obs()['observation'])
                        else:
                ...
                flush(obs, actions, scores, pobs)
                for x in obs: x.clear()
                for x in actions: x.clear()
                for x in pobs: x.clear()
    print(f'total matches {matchid+1}  post samples {len(post_obs)}', flush=True)
    obs_a = np.stack(post_obs).reshape((-1, 38, 4, 9)).astype(np.int8)
    score_a = np.array(post_score, dtype=np.float32)
    place_a = np.array(post_place, dtype=np.int8)
    print(f'shapes obs {obs_a.shape} score {score_a.shape} place {place_a.shape}', flush=True)
    for pl in (1, 2, 3, 4):
        print(f'  place=={pl}: {float((place_a == pl).mean()):.4f}', flush=True)
    np.savez_compressed(OUT, obs=obs_a, deal_score=score_a, deal_place=place_a)
