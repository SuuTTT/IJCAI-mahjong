...
def main():
    ...
    # SE of edge_per_game: each seed is an independent duplicate unit contributing
    # combined delta d_i = (a_cand - a_ref) + (b_cand - b_ref) over 2 games.
    # edge_per_game = mean(d_i)/2 ; SE = std(d_i)/sqrt(n)/2
    import numpy as _np
    d=_np.array([(r[0]-r[1])+(r[2]-r[3]) for r in res],dtype=_np.float64)
    n=len(d)
    edge_se=float(d.std(ddof=1)/_np.sqrt(n)/2.0) if n>1 else 0.0
    dA=_np.array([r[0]-r[1] for r in res],dtype=_np.float64)
    dB=_np.array([r[2]-r[3] for r in res],dtype=_np.float64)
    seatA_se=float(dA.std(ddof=1)/_np.sqrt(n)) if n>1 else 0.0
    seatB_se=float(dB.std(ddof=1)/_np.sqrt(n)) if n>1 else 0.0
    ci_lo=edge-1.96*edge_se; ci_hi=edge+1.96*edge_se
    out=dict(cand=os.path.basename(a.cand),ref=os.path.basename(a.ref),
             ...
             edge_se=round(edge_se,4), ci95_lo=round(ci_lo,4), ci95_hi=round(ci_hi,4),
             seatA_se=round(seatA_se,4), seatB_se=round(seatB_se,4),
