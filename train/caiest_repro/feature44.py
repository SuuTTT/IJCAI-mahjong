...
class FeatureAgent(MahjongGBAgent):
    ...
    OFFSET_OBS = {
        ...
        # 每个位置的副露
        'HALF_FLUSH' : 22,
        # ---- game-phase features (planes 38..43), appended; first 38 planes IDENTICAL to 38-plane feature ----
        # dead-count thermometer: 4 planes = total visible copies (0..4) of each tile
        'DEAD' : 38,
        # wall-remaining: 1 plane, normalized to [0,1], broadcast across grid
        'WALL' : 42,
        # turn-count: 1 plane, normalized to [0,1], broadcast across grid
        'TURN' : 43,
    }
    WALL_TOTAL = 84  # 4 seats * 21 wall tiles at deal
    TURN_NORM  = 50.0  # typical max number of draw turns to normalize by
    # 给每种动作编了号
    OFFSET_ACT = {
        'Pass' : 0,
        'Hu' : 1,
    ...
    def __init__(self, seatWind):
        ...
        # game-phase: count of draw events seen (any player), normalized for the TURN plane
        self.turnCount = 0
    ...
    # ---- game-phase planes (computed fresh at obs time) ----
    def _fill_phase_planes(self):
        # DEAD: thermometer of total visible copies (0..4) per tile =
        #   shownTiles[tile] (all players' discards + melds) + my hand count.
        do = self.OFFSET_OBS['DEAD']
        self.obs[do:do+4] = 0
        d = defaultdict(int)
        for tile in self.hand:
            d[tile] += 1
        for tile in self.OFFSET_TILE:
            cnt = self.shownTiles[tile] + d[tile]
            if cnt > 4: cnt = 4
            if cnt > 0:
                self.obs[do:do+cnt, self.OFFSET_TILE[tile]] = 1
        # WALL: tiles remaining across all seats' walls, normalized, broadcast over grid
        wall_rem = sum(self.tileWall)
        self.obs[self.OFFSET_OBS['WALL']] = min(1.0, max(0.0, wall_rem / self.WALL_TOTAL))
        # TURN: draw count normalized, broadcast over grid
        self.obs[self.OFFSET_OBS['TURN']] = min(1.0, self.turnCount / self.TURN_NORM)

    # 返回当前观测到的状态：观测、动作mask
    def _obs(self):
        self._fill_phase_planes()
