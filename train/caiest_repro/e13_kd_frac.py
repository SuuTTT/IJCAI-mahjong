...
def main():
    ...
    if a.frac < 1.0:
        _r=np.random.RandomState(9000+a.seed); _p=_r.permutation(len(tidx)); tidx=tidx[_p[:max(256,int(len(tidx)*a.frac))]]
        print('FRAC %.3f -> train %d'%(a.frac,len(tidx)),flush=True)
