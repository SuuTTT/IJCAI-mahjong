...
from feature import FeatureAgent as CaiAgent   # caiest (38,4,9) feature, for the CNN policy
from feature44 import FeatureAgent as CaiAgent44  # extended (44,4,9) game-phase feature
try:
...
class Sim:
    ...
    def __init__(self, policy_fn, seed=0, quan=0, learner_seats=None, cnn=True,
                 seed_hand=None, seed_seat=0, seat_planes=None):
        ...
        self.cai = None
        self.cai44 = None
        # seat_planes[s] in {38,44}: which feature encoding to feed seat s's policy.
        # Default all-38 (original behaviour). Method (a): mix 38 (moyu) and 44 (cand).
        self.seat_planes = [38]*4 if seat_planes is None else list(seat_planes)
        self._need44 = any(p == 44 for p in self.seat_planes)
    ...
    def _cai(self, msg):
        """Feed a LOGICAL game event to the caiest shadow agents (once each)."""
        if self.cai is None: return
        for s in range(4):
            try: self.cai[s].request2obs(msg)
            except Exception: pass
        if self.cai44 is not None:
            for s in range(4):
                try: self.cai44[s].request2obs(msg)
                except Exception: pass
    ...
    def reset(self):
        ...
        self.cai44 = [CaiAgent44(s) for s in range(4)] if (self.cnn and self._need44) else None
        for s in range(4):
            self.agents[s].update(f"Wind {self.quan}")
            if self.cai is not None: self.cai[s].request2obs(f"Wind {self.quan}")
            if self.cai44 is not None: self.cai44[s].request2obs(f"Wind {self.quan}")
        for s in range(4):
            self.agents[s].update("Deal " + " ".join(self.hand[s]))
            if self.cai is not None: self.cai[s].request2obs("Deal " + " ".join(self.hand[s]))
            if self.cai44 is not None: self.cai44[s].request2obs("Deal " + " ".join(self.hand[s]))
    ...
    def _obs_mask(self, seat):
        if self.cnn and self.cai is not None:
            if self.seat_planes[seat] == 44 and self.cai44 is not None:
                d = self.cai44[seat]._obs()
            else:
                d = self.cai[seat]._obs()
            return d['observation'].copy(), d['action_mask'].astype(bool)
        a = self.agents[seat]
    ...
    def _broadcast(self, msg):
        ...
        self._cai(msg)   # caiest interprets "Player N ..." relative to each seat
    ...
    def _loop(self, max_turns):
        ...
        for _ in range(max_turns):
            ...
            if self.cai44 is not None:
                self.cai44[cur].request2obs(f"Draw {t}")
                for s in range(4):
                    if s != cur: self.cai44[s].request2obs(f"Player {cur} Draw")
