"""
e8_gate.py -- E8: VALUE-GUIDED 1-ply action selection vs the base policy (cnn_lad),
...
def _apply_for_value(agent, a):
    ...
    if PLAY <= a < CHI0:                # DISCARD
        ...
        try:
            o = ag2.request2obs("Player %d Play %s" % (seat, tile))
...
def value_guided_action(lg, mask, agent, vnet, lam, topk, cur_obs):
...
class PlacementSim(Sim):
    ...
    def _ask(self, seat):
        ...
        if seat == self.cand_seat and self.lam > 0:
            ...
            act = value_guided_action(lg, mask, agent, self.vnet, self.lam, self.topk, obs)
