"""preprocess_ext44.py — same extraction as preprocess_single.py but with the 44-plane
feature44 encoder. Stores obs as int8: planes 0-41 are binary (0/1); the WALL (42) and
TURN (43) planes are fractional [0,1] -> stored *100 as int8 (0..100), divide by 100 at train
time. First 38 planes are byte-identical to cooked_single.npz.
"""
import os, sys
sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
import numpy as np
from feature44 import FeatureAgent

OUT = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'data', 'cooked_ext44.npz')
...
def _quant(o):
    # o: (44,4,9) float. binary planes already 0/1; scale wall/turn to 0..100 for int8 storage.
    o = o.copy()
    o[WALL_P] = np.round(o[WALL_P] * 100)
    o[TURN_P] = np.round(o[TURN_P] * 100)
    ...
    for j in range(4):
        for i, a in enumerate(actions[j]):
            ...
            if np.sum(o['action_mask']) != 1:
                all_obs.append(_quant(o['observation']))
...
def main():
    ...
    obs_a = np.stack(all_obs).reshape((-1, 44, 4, 9)).astype(np.int8)
    mask_a = np.stack(all_mask).astype(np.bool_)
    act_a = np.array(all_act, dtype=np.int16)
    print(f'shapes obs {obs_a.shape} mask {mask_a.shape} act {act_a.shape}  act min/max {act_a.min()}/{act_a.max()}', flush=True)
