...
KNOBS = {
    ...
    "targeted":   (0,           99,      9,        0,     99,       99),   # fan-weighted cost-gated (uses TD.TAU)
}
...
def apply_knob(agent, lg, mask):
    ...
    if _TD_ON:
        chosen_tile = TD.choose_discard(agent, ranked_tiles, top_k=TD.TOP_K)
    else:
...
def _work(arg):
    global _TD_ON
    seed, cand, ck, ccfg, ref, rk, rcfg, knob, tau, td_topk, td_minturn = arg
    SD._configure(*KNOBS[knob])
    _TD_ON = (knob == "targeted")
    if _TD_ON:
        TD.TAU = tau; TD.TOP_K = td_topk; TD.MIN_TURN = td_minturn
        for kk in TD.STATS: TD.STATS[kk] = 0 if isinstance(TD.STATS[kk], int) else 0.0
    ...
    for cs in range(4):
        ...
        pts[r] += 1
        seat_pts[cs][r] += 1
    td_stats = dict(TD.STATS) if _TD_ON else None
    return placement_sum, pts, micro_cand, seat_pts, td_stats
...
def main():
    ...
    ap.add_argument("--td-topk", type=int, default=4)
    ap.add_argument("--td-minturn", type=int, default=6)
    ...
    td_agg = {"discards": 0, "fired": 0, "noop_same": 0, "cost_sum": 0.0, "cost_fired_sum": 0.0}
    have_td = False
    for r in res:
        if len(r) > 4 and r[4] is not None:
            have_td = True
            for kk in td_agg: td_agg[kk] += r[4][kk]
    ...
    out = dict(
        ...
        tau=a.tau if a.knob == "targeted" else None,
        td_topk=a.td_topk, td_minturn=a.td_minturn,
        td_discards=td_agg["discards"] if have_td else None,
        td_fired=td_agg["fired"] if have_td else None,
        td_noop_same=td_agg["noop_same"] if have_td else None,
        td_fire_pct=round(100.0 * td_agg["fired"] / td_agg["discards"], 3) if (have_td and td_agg["discards"]) else None,
        td_mean_cost=round(td_agg["cost_sum"] / td_agg["discards"], 4) if (have_td and td_agg["discards"]) else None,
        td_mean_cost_fired=round(td_agg["cost_fired_sum"] / td_agg["fired"], 4) if (have_td and td_agg["fired"]) else None,
